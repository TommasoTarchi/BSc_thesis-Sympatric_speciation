//Dieckman e Doebeli per organimsi diploidi con solo crattere ecologico e assortatività basata su carattere ecologico (4 cromosomi)
...
//distribuzioni iniziali: delta in f=0 per E, delta in f=L/2 per A
...
int main(){
	...
	L=10;  //lunghezza del singolo cromosoma
	T=100;  //generazioni
	b=50;
	r=10;
	J=70;
	...
	genotipi_delta(gA);  //inizializza a delta in f=L/2 i genotipi assortativi
	fenotipi(gE, fE);
	fenotipi(gA, fA);
	...
	FILE *dati;
	dati=fopen("DD.dat", "w");
	fprintf(dati, "#fenotipo distribuzione_ecologico distribuzione_assortativo:\n");
	for(i=0; i<L+1; i++){
		fprintf(dati, "%d   %f   %f\n", i, *(pE+i), *(pA+i));
	}
	//fprintf(dati, "\n");
...
int mating(int v, int fAv, int *fE, double *c){
	...
	if(fAv==L/2){  //calcolo della "compatibilità"
		...
		sigma=1.0/(m*m);
		//sigma=10;
		c[0]=1-0.95*exp(-pow((double)fE[i]-(double)fE[v], 2.0)/(2.0*sigma*sigma));
		for(i=1; i<N; i++){
			c[i]=c[i-1]+1-0.95*exp(-pow((double)fE[i]-(double)fE[v], 2.0)/(2.0*sigma*sigma));
		}
	}
